...
<section id="hero">
	...
	<div class="hero-dots">
		{#each slides as _, i}
			<button class="hero-dot" class:active={currentSlide === i} onclick={() => (currentSlide = i)} aria-label="Go to slide {i + 1}"></button>
		{/each}
	</div>
...
<style>
	...
	.hero-slide-cta {
		...
		border-radius: 24px;
		opacity: 0;
		transition: opacity 0.5s ease 0.5s, background 0.3s, border-color 0.3s;
	}
